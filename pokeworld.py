...
pokemonWorld = {
    ...
    'charmander':   {'type': 'fire',        'evolveAt': 18,     'evolveTo': 'charmeleon',   'baseDef': 15,  'baseSpeed': 20, 'startAttacks': [deepcopy(ember), deepcopy(tackle), None, None],                     'learnableAttacks': [deepcopy(swift), deepcopy(flamethrower), deepcopy(firepunch), deepcopy(firespin), deepcopy(shelltrap), deepcopy(fireblast), deepcopy(sacredfire)][::-1]       },           
    'squirtle':     {'type': 'water',       'evolveAt': 18,     'evolveTo': 'wartortle',    'baseDef': 24,  'baseSpeed': 21, 'startAttacks': [deepcopy(watergun), deepcopy(tackle), None, None],                    'learnableAttacks': [deepcopy(headbutt), deepcopy(bubblebeam), deepcopy(mudslap), deepcopy(hydropump), deepcopy(watercannon), deepcopy(hydrovortex)][::-1]         },           
    'bulbasaur':    {'type': 'grass',       'evolveAt': 18,     'evolveTo': 'ivysaur',      'baseDef': 15,  'baseSpeed': 20, 'startAttacks': [deepcopy(razorleaf), deepcopy(tackle), None, None],                   'learnableAttacks': [deepcopy(vinewhip), deepcopy(disable), deepcopy(stunspore), deepcopy(gigadrain), deepcopy(frenzy), deepcopy(leafblade), deepcopy(solarbeam)][::-1]         },           
